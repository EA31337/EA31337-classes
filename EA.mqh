...
class EA : public Taskable<DataParamEntry> {
 ...
 public:
  ...
  float GetSignalOpenFiltered(StrategySignal &_signal, unsigned int _sf) {
    float _result = _signal PTR_DEREF GetSignalOpen();
    ENUM_TIMEFRAMES _sig_tf = _signal PTR_DEREF Get<ENUM_TIMEFRAMES>(STRUCT_ENUM(StrategySignal,
  STRATEGY_SIGNAL_PROP_TF)); if (ChartTf::TfToHours(_sig_tf) < 1 && bool(_sf & STRUCT_ENUM(EAParams,
  EA_PARAM_SIGNAL_FILTER_OPEN_M_IF_H))) { _result = 0; int64 _tfts[4]; _tfts[0] = ChartStatic::iTime(_Symbol,
  PERIOD_H1); _tfts[1] = ChartStatic::iTime(_Symbol, PERIOD_H4); _tfts[2] = ChartStatic::iTime(_Symbol, PERIOD_H1, 1);
  ...
  */
   ...
   */
  DictStruct<int64, Ref<Strategy>> *GetStrategies() { return GetPointer(strats); }
