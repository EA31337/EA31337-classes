...
struct TradeSignalEntry {
 ...
 public:
  ...
  char GetCloseDirection() {
    if (CheckSignals(TRADE_SIGNAL_FLAG_CLOSE_BUY_MAIN & ~TRADE_SIGNAL_FLAG_CLOSE_SELL_MAIN)) {
      return 1;
    } else if (CheckSignals(TRADE_SIGNAL_FLAG_CLOSE_SELL_MAIN & ~TRADE_SIGNAL_FLAG_CLOSE_BUY_MAIN)) {
      return -1;
    }
    return 0;
  }
  char GetOpenDirection() {
    if (CheckSignals(TRADE_SIGNAL_FLAG_OPEN_BUY_MAIN & ~TRADE_SIGNAL_FLAG_OPEN_SELL_MAIN)) {
      return 1;
    } else if (CheckSignals(TRADE_SIGNAL_FLAG_OPEN_SELL_MAIN & ~TRADE_SIGNAL_FLAG_OPEN_BUY_MAIN)) {
      return -1;
    }
    return 0;
  }
