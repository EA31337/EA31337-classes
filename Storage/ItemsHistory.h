  ...
  */
  ...
  void RegenerateHistory(int _from_index, int _to_index, ENUM_ITEMS_HISTORY_DIRECTION _dir) {
    ...
    if (_dir == ITEMS_HISTORY_DIRECTION_FORWARD) {
      if (history.Size() == 0) {
        ...
        Print("Getting last valid item at index ", last_valid_index);
      ...
      if (history.Size() == 0) {
        ...
        Print("Getting first valid item at index ", first_valid_index);
    ...
    if (ArraySize(_items) != _item_count) {
      // There's really no problem if number of generated items are less that requested.
      // @todo However, if there's to many calls for RegenerateHistory then we need to find a way to make it to exit
      // earlier.
      Print("Regeneration failed. Requested ", _item_count, " items, got ", ArraySize(_items),
  ...
  bool TryGetItemByIndex(int _index, IV& _out_item, bool _try_regenerate = true) {
    if (history.Size() == 0 || _index < first_valid_index || _index > last_valid_index) {
      if (!_try_regenerate) {
        Print("Missing history. Tried to get item at index ", _index);
        return false;
      }
