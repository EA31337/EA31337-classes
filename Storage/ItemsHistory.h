...
class ItemsHistoryItemProvider : public Dynamic {
 public:
  ...
  virtual long GetInitialTimeMs() {
    Print("Error: ItemsHistory item's provider does not implement GetInitialTimeMs(), but it should!");
    DebugBreak();
    return 0;
  }
};
  ...
  */
  ...
  void RegenerateHistory(int _from_index, int _to_index, ENUM_ITEMS_HISTORY_DIRECTION _dir) {
    ...
    if (ArraySize(_items) != _item_count) {
      // @todo There's really no problem if number of generated items are less that requested.
