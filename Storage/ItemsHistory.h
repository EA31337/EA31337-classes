...
/**
 * Whether we're selecting history via index (shift) or time in milliseconds.
 */
enum ENUM_ITEMS_HISTORY_SELECTOR { ITEMS_HISTORY_SELECTOR_INDEX, ITEMS_HISTORY_SELECTOR_TIME_MS };
...
class ItemsHistoryItemProvider : public Dynamic {
  ItemsHistory<IV, ItemsHistoryItemProvider<IV>>* history;

 public:
  /**
   * Constructor.
   */
  ItemsHistoryItemProvider(ItemsHistory<IV, ItemsHistoryItemProvider<IV>>* _history) : history(_history) {}
   ...
   */
  void GetItems(long _from, ENUM_ITEMS_HISTORY_SELECTOR _sel, datetime _to_ms, ARRAY_REF(IV, _out_arr)) {
    // @todo.
  }

  /**
   * Returns pointer to items history this item providers works for.
   */
  ItemsHistory<IV, ItemsHistoryItemProvider<IV>>* GetHistory() { return history; }
};
...
class ItemsHistory {
  ...
  */
  ...
  void RegenerateHistory(int _from_index, int _to_index, ENUM_ITEMS_HISTORY_DIRECTION _dir) {
    static ARRAY(IV, _items);  // Items generated by provider.

    ArrayResize(_items, 0);

    int _item_count = _to_index - _from_index + 1;

    item_provider REF_DEREF GetItems(_from_index, ITEMS_HISTORY_SELECTOR_INDEX, _dir, _item_count, _items);
  ...
  void Prepend(IV& _item, bool _allow_regenerate = true) {
    ...
    // Adding item at the beginning of all the history and expanding history by one item in the past.
