...
class ItemsHistoryItemProvider : public Dynamic {
 public:
   ...
   */
  void GetItems(ItemsHistory<IV, ItemsHistoryItemProvider<IV>>* _history, long _from, ENUM_ITEMS_HISTORY_SELECTOR _sel,
...
class ItemsHistory {
  ...
  */
  ...
  void RegenerateHistory(int _from_index, int _to_index, ENUM_ITEMS_HISTORY_DIRECTION _dir) {
    ...
    item_provider REF_DEREF GetItems(THIS_PTR, _from_index, ITEMS_HISTORY_SELECTOR_INDEX, _dir, _item_count, _items);

    if (ArraySize(_items) != _item_count) {
      Print("Error: Wrong number of items generated by item provider. Requested ", _item_count, " items, but got ",
            ArraySize(_items));
      DebugBreak();
      return;
    }
  ...
  bool TryGetItemByIndex(int _index, IV& _out_item, bool _try_regenerate = true) {
    if (history.Size() == 0 || _index < first_valid_index || _index > last_valid_index) {
      ...
      RegenerateHistory(_index < first_valid_index ? _index : last_valid_index,
                        _index > last_valid_index ? _index : last_valid_index, _dir);
