...
#ifdef __MQL4__
#define TFS 9
const ENUM_TIMEFRAMES tf[TFS] = {PERIOD_M1, PERIOD_M5, PERIOD_M15, PERIOD_M30, PERIOD_H1,
...
#define TFS 21
const ENUM_TIMEFRAMES tf[TFS] = {PERIOD_M1,  PERIOD_M2,  PERIOD_M3,  PERIOD_M4,  PERIOD_M5, PERIOD_M6, PERIOD_M10,
...
int GetOpenMethod(const int open_method, const int one_of_methods, const int sum_of_methods) {
  ...
  if (open_method == OPEN_METHOD_ONE) result = -one_of_methods;
...
//|                                                                  |
//+------------------------------------------------------------------+
string TimeframeToString(const ENUM_TIMEFRAMES _tf) { return (StringSubstr(EnumToString(_tf), 7)); }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
string OpenMethodToString(const ENUM_OPEN_METHOD _open_method, const int one_of_methods, const int sum_of_methods) {
  string result = "";
  switch (_open_method) {
    case OPEN_METHOD_ONE:
      result = "One Of Methods (" + IntegerToString(one_of_methods) + ")";
      break;
    case OPEN_METHOD_SUM:
      result = "Sum Of Methods (" + IntegerToString(sum_of_methods) + ")";
      break;
    default:
      result = StringSubstr(EnumToString(_open_method), 5);
      break;
  }
  return (result);
}
...
class CBasicTrade {
 ...
 protected:
  ...
  double NormalizeVolume(const double _volume) {
    ...
    if (lot_value <= lot_min)
      lot_value = lot_min;
    else if (lot_value >= lot_max)
      lot_value = lot_max;
    else
      lot_value = round(lot_value / lot_step) * lot_step;
    //---
  ...
  int TimeframeToIndex(ENUM_TIMEFRAMES _tf) {
    if (_tf == 0 || _tf == PERIOD_CURRENT) _tf = (ENUM_TIMEFRAMES)_Period;
    int total = ArraySize(tf);
    for (int i = 0; i < total; i++) {
      if (tf[i] == _tf) return (i);
    }
    return (0);
  }
 ...
 public:
  //+------------------------------------------------------------------+
  void CBasicTrade(void) { m_last_error = 0; }
  ...
  bool Trade(string _symbol,              // symbol
    ...
    //--- get coef point
    int _coef_point = 1;
    if (_digits == 3 || _digits == 5) _coef_point = 10;
    ...
    while (attempts > 0) {
      ...
      //---
      double price = 0.0;
      if (_type == OP_BUY) price = NormalizeDouble(SymbolInfoDouble(_symbol, SYMBOL_ASK), _digits);
      if (_type == OP_SELL) price = NormalizeDouble(SymbolInfoDouble(_symbol, SYMBOL_BID), _digits);
          ...
          if (OrderSelect(ticket, SELECT_BY_TICKET)) {
            ...
            while (attempts > 0) {
              ...
              if (OrderType() == OP_BUY) {
                if (_stop_loss == -1.0)
                  sl = order_stop_loss;
                else if (_stop_loss == 0.0)
                  sl = 0.0;
                else
                  sl = NormalizeDouble(
                      fmin(order_open_price - _stop_loss * _coef_point * _point, _bid - stop_level * _point), _digits);

                if (_take_profit == -1.0)
                  tp = order_take_profit;
                else if (_take_profit == 0.0)
                  tp = 0.0;
                else
                  tp = NormalizeDouble(
                      fmax(order_open_price + _take_profit * _coef_point * _point, _bid + stop_level * _point),
                      _digits);
              }
              ...
              if (OrderType() == OP_SELL) {
                if (_stop_loss == -1.0)
                  ...
                  sl = NormalizeDouble(
                      fmax(order_open_price + _stop_loss * _coef_point * _point, _ask + stop_level * _point), _digits);
                ...
                if (_take_profit == -1.0)
                  ...
                  tp = NormalizeDouble(
                      fmin(order_open_price - _take_profit * _coef_point * _point, _ask - stop_level * _point),
                      _digits);
              }

              if (sl == order_stop_loss && tp == order_take_profit) return (true);
