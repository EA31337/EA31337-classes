...
public:
   ...
   * Constructor.
   */
  JsonNode(JsonNodeType type, JsonNode* parent = NULL, JsonParam* key = NULL, JsonParam* value = NULL) : _type(type), _parent(parent), _key(key), _value(value), _numChildren(0), _currentChildIndex(0) {
  }
   ...
   */
  string Key() {
    return _key != NULL ? _key.AsString(false, false) : "";
  }

  /**
   * Returns pointer to JsonParam holding the key or NULL.
   */
  JsonParam* GetKeyParam() {
    return _key;
  }
   ...
   */
  void SetType(JsonNodeType type) {
    _type = type;
  }
  ...
  string ToString(bool trimWhitespaces = false, unsigned int indentSize = 2, unsigned int indent = 0) {
    ...
    if (!trimWhitespaces) 
      for (unsigned int i = 0; i < indent * indentSize; ++i)
        ident += " ";
    ...
    if (GetValueParam() != NULL)
      repr += GetValueParam().AsString(false, true);

    switch (GetType()) {
      case JsonNodeObject: repr += "{" + (trimWhitespaces ? "" : "\n"); break;
      case JsonNodeArray: repr += "[" + (trimWhitespaces ? "" : "\n"); break;
    }
    ...
    switch (GetType()) {
      case JsonNodeObject: repr += ident + "}"; break;
      case JsonNodeArray: repr += ident + "]"; break;
    }
