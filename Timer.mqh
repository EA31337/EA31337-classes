...
class Timer : public Object {
 ...
 public:
  ...
  Timer *PrintOnMax(ulong _min = 1) {
    return data[index] > _min && data[this.index] >= this.max ? PrintSummary() : GetPointer(this);
  }
  ...
  uint GetTime() { return GetTickCount() - this.start; }
   ...
   */
  uint GetMin() { return this.index >= 0 ? this.data[ArrayMinimum(this.data)] : 0; }
  ...
  virtual string ToString() {
    return StringFormat("%s(%d)=%d-%dms,med=%dms,sum=%dms", GetName(), ArraySize(this.data), GetMin(), GetMax(),
                        GetMedian(), GetSum());
  }
