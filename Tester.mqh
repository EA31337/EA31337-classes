...
class Tester : public Terminal {
 public:
  ...
  static bool ValidSpread(string _symbol = NULL, bool verbose = true) {
    unsigned int _symbol_spread = SymbolInfo::GetSpread(_symbol);
    unsigned int _real_spread = SymbolInfo::GetRealSpread(_symbol);
    double _lot_step = SymbolInfo::GetVolumeStep(_symbol);
    unsigned int _digits = SymbolInfo::GetDigits(_symbol);
