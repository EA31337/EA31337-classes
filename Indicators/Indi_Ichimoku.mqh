  ...
  double GetValue(ENUM_ICHIMOKU_LINE _mode, int _shift = 0) {
    ...
    switch (params.idstype) {
      case IDATA_BUILTIN:
        ...
        _value = Indi_Ichimoku::iIchimoku(Get<string>(CHART_PARAM_SYMBOL), Get<ENUM_TIMEFRAMES>(CHART_PARAM_TF), GetTenkanSen(), GetKijunSen(), GetSenkouSpanB(), _mode,
                                          _shift, GetPointer(this));
        break;
      case IDATA_ICUSTOM:
        _value = iCustom(istate.handle, Get<string>(CHART_PARAM_SYMBOL), Get<ENUM_TIMEFRAMES>(CHART_PARAM_TF), params.GetCustomIndicatorName(), /*[*/ GetTenkanSen(),
