...
class Indi_Drawer : public Indicator {
 public:
   ...
   * Class constructor.
   */
  Indi_Drawer(const DrawerParams &_params, ENUM_TIMEFRAMES _tf = PERIOD_CURRENT)
      : Indicator((IndicatorParams)_params, _tf), redis(true) {
    params = _params;
    Init();
  }
  Indi_Drawer(ENUM_TIMEFRAMES _tf = PERIOD_CURRENT) : Indicator(INDI_DRAWER, _tf), redis(true) { Init(); }
  ...
  double GetValue(int _mode = 0, int _shift = 0) {
    ...
    switch (params.idstype) {
      ...
      case IDATA_INDICATOR:
        _value = Indi_Drawer::iDrawerOnIndicator(params.indi_data_source, THIS_PTR, GetSymbol(), GetTf(), GetPeriod(),
