    ...
    switch (iparams.idstype) {
      case IDATA_BUILTIN:
        ...
        _value = Indi_Momentum::iMomentum(Get<string>(CHART_PARAM_SYMBOL), Get<ENUM_TIMEFRAMES>(CHART_PARAM_TF),
                                          GetPeriod(), GetAppliedPrice(), iparams.shift + _shift, GetPointer(this));
        break;
      case IDATA_ICUSTOM:
      ...
      case IDATA_INDICATOR:
        ...
        // @fixit Somehow shift isn't used neither in MT4 nor MT5.
        _value = Indi_Momentum::iMomentumOnIndicator(indi_src, Get<string>(CHART_PARAM_SYMBOL),
                                                     Get<ENUM_TIMEFRAMES>(CHART_PARAM_TF), GetPeriod(),
                                                     GetDataSourceMode(), iparams.GetShift() + _shift);
        if (iparams.is_draw) {
