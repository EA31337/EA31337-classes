...
struct IndiKillzonesParams : IndicatorParams {
  ...
  void IndiKillzonesParams(int _shift = 0, ENUM_TIMEFRAMES _tf = PERIOD_CURRENT) {
    itype = INDI_PIVOT;
    max_modes = FINAL_INDI_KILLZONES_MODE_ENTRY;
...
class Indi_Killzones : public Indicator {
 protected:
  ...
  IndiKillzonesParams params;

 public:
  /**
   * Class constructor.
   */
  Indi_Killzones(IndiKillzonesParams &_params) : params(_params), Indicator((IndicatorParams)_params){};
  ...
  IndicatorDataEntry GetEntry(int _shift = 0) {
    ...
    if (!_entry.IsValid() && !_entry.CheckFlag(INDI_ENTRY_FLAG_INSUFFICIENT_DATA)) {
      ...
      for (unsigned int _mode = 0; _mode < params.max_modes; _mode++) {
