   ...
   public:                                                                                                     \
    static T Get(const string Symb, const int TimeFrame, const int iShift) {                                   \
      ...
      return ((Copy##FUNC((Symb == NULL) ? _Symbol : Symb, _Period, iShift, 1, tValue) > 0) ? tValue[0] : -1); \
                                                                                                               ...
                                                                                                               \
    T operator[](const int iPos) const { return (CLASS##NAME::Get(_Symbol, _Period, iPos)); }                  \
  };                                                                                                           \
                                                                                                               \
  CLASS##NAME NAME;                                                                                            \
                                                                                                               \
...
#undef WHILE
 ...
 public:
  ...
  static int MT4OrderSend(const string Symb, const int Type, const double dVolume, const double Price,
  ...
  static bool MT4OrderModify(const ulong Ticket, const double Price, const double SL, const double TP,
    ...
    bool Res =
        ((Ticket != MT4ORDERS::Order.Ticket) || (MT4ORDERS::Order.Ticket <= OP_SELL))
            ? (MT4ORDERS::ModifyPosition(Ticket, Request) ? true
                                                          : MT4ORDERS::ModifyOrder(Ticket, Price, Expiration, Request))
            : (MT4ORDERS::ModifyOrder(Ticket, Price, Expiration, Request) ? true
  ...
  static bool MT4OrderClose(const ulong Ticket, const double dLots, const double Price, const int SlipPage,
...
bool OrderClose(const ulong Ticket, const double dLots, const double Price, const int SlipPage,
                const color Arrow_Color = clrNONE) {
  return (MT4ORDERS::MT4OrderClose(Ticket, dLots, Price, SlipPage, Arrow_Color));
}

bool OrderModify(const ulong Ticket, const double Price, const double SL, const double TP, const datetime Expiration,
                 const color Arrow_Color = clrNONE) {
